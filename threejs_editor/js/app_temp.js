// ========================================
// PREVIEW MODE - CON MODAL MP4, MORE E 3D
// ========================================
...
import { RoutesFooter } from './routes-footer.js';`n
export class PreviewMode {
 ...
 this.contentModal = null;`n this.routesFooter = null;
 ...
 this.contentModal = new ContentModal();`n `n // Init Routes Footer`n this.routesFooter = new RoutesFooter();`n this.routesFooter.create();
 ...
 start(allRoutes, startRouteName = null) { // Accept all routes`n if (!allRoutes || typeof allRoutes !== 'object') {`n alert('Nessun percorso disponibile!');`n return;`n }`n `n // If allRoutes is a single route (old format), convert it`n if (allRoutes.punti) {`n allRoutes = { 'Percorso': allRoutes };`n startRouteName = 'Percorso';`n }`n `n const routeNames = Object.keys(allRoutes);`n if (routeNames.length === 0) {`n alert('Nessun percorso disponibile!');`n return;`n }`n `n // Select starting route`n const initialRouteName = startRouteName || routeNames[0];`n const percorso = allRoutes[initialRouteName];`n `n if (!percorso || percorso.punti.length === 0) {
 ...
 console.log(`📍 [${index + 1}] Tipo: ${h.tipo}, Position:`, position);
 ...
 // Open 3D modal (using ContentModal for Sketchfab)
 if (content) {
 this.contentModal.open(content);
 } else {
 alert('🗿 Hotspot 3D\n\nNessun contenuto configurato per questo hotspot.');
